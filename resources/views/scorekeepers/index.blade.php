<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-h-wv text-wv-text leading-tight">{{ __('Anotadores') }}</h2>
                <p class="text-sm text-wv-text-secondary mt-1">{{ __('Personas que pueden llevar el control de juegos.') }}</p>
            </div>
            <a href="{{ route('scorekeepers.create') }}" class="inline-flex items-center px-4 py-2 bg-wv-accent hover:bg-wv-accent-hover text-wv-text-on-accent text-sm font-semibold rounded-card transition">
                + {{ __('Nuevo anotador') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- DISI-67: stats --}}
            @php $statLabels = ['total' => __('Total'), 'active' => __('Activos'), 'inactive' => __('Inactivos'), 'games' => __('Juegos anotados')]; @endphp
            @if (! empty($stats))
            ...
            <div class="bg-wv-surface border border-wv-border rounded-card overflow-hidden">
                ...
                @else
                                ...
                                @foreach ($scorekeepers as $sk)
                                    <tr class="hover:bg-wv-surface-hover transition">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($sk->photoUrl)
                                                <img src="{{ $sk->photoUrl }}" class="h-10 w-10 rounded-full object-cover bg-wv-surface">
                                            @else
                                                <div class="h-10 w-10 rounded-full bg-wv-surface-hover flex items-center justify-center text-wv-text-secondary text-xs font-semibold">
                                                    {{ mb_substr($sk->first_name, 0, 1) }}{{ mb_substr($sk->last_name, 0, 1) }}
                                                </div>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <div class="px-6 py-3 border-t border-wv-border">{{ $scorekeepers->links() }}</div>
                @endif
            </div>
        </div>
    </div>
