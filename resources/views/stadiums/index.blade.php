                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Ciudad / Estado') }}</th>
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Capacidad') }}</th>
                            ...
                            @foreach ($stadiums as $st)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('stadiums.show', $st) }}" class="text-indigo-600 hover:text-indigo-800 font-medium">
                                            {{ $st->name }}
                                        </a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ trim(($st->city ?? '') . (($st->city && $st->state) ? ', ' : '') . ($st->state ?? ''), ', ') ?: '—' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                        {{ $st->capacity ? number_format($st->capacity, 0, ',', '.') : '—' }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('stadiums.edit', $st) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                            {{ __('Editar') }}
                                        </a>
                                        <form action="{{ route('stadiums.destroy', $st) }}" method="POST" class="inline"
                                              onsubmit="return confirm('¿Eliminar el estadio «{{ $st->name }}»?');">
                                            @csrf
                                            @method('DELETE')
