<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-wv-text-secondary mt-1">{{ __('Modifica los datos del torneo.') }}</p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 flex gap-3 text-sm">
                <a href="{{ route('tournaments.index') }}" class="text-wv-accent hover:text-wv-accent-hover">
                    ← {{ __('Volver al listado') }}
                </a>
                <a href="{{ route('tournaments.show', $tournament) }}" class="text-wv-accent hover:text-wv-accent-hover">
                    {{ __('Ver detalle') }}
                </a>
            </div>
