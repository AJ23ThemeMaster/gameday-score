                ...
                @if ($teams->isEmpty())
                    <div class="p-10 text-center text-wv-text-secondary">
                        @auth
                            ...
                            @else
                                <p class="mb-4">{{ __('A√É¬∫n no hay equipos registrados.') }}</p>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            @auth
                                                @if (auth()->user()->isAdmin())
                                                    <form action="{{ route('teams.destroy', $team) }}" method="POST" class="inline" data-confirm="'øEliminar el equipo ´{{ $team->name }}ª?'" data-confirm-danger="true" data-loader>
                                                        @csrf @method('DELETE')
