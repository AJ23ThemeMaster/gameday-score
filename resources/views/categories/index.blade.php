<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-h-wv text-wv-text leading-tight">{{ __('Categor√É≠as') }}</h2>
                <p class="text-sm text-wv-text-secondary mt-1">{{ __('Reglas por categor√É≠a (innings, nocaut, pitch limit).') }}</p>
            </div>
            ...
            <div class="bg-wv-surface border border-wv-border rounded-card overflow-hidden">
                @if ($categories->isEmpty())
                    <div class="p-10 text-center text-wv-text-secondary">
                        <p class="mb-4">{{ __('A√É¬∫n no hay categor√É≠as registradas.') }}</p>
                        <a href="{{ route('categories.create') }}" class="text-wv-accent hover:text-wv-accent-hover underline">{{ __('Crear la primera categor√É≠a') }}</a>
                    </div>
                @else
                            ...
                            <tbody class="divide-y divide-wv-border">
                                @foreach ($categories as $cat)
                                    <tr class="hover:bg-wv-surface-hover transition">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-wv-text">{{ $cat->pitch_limit ?? '√¢‚Ç¨‚Äù' }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            <form action="{{ route('categories.destroy', $cat) }}" method="POST" class="inline" data-confirm="'øEliminar la categor√É≠a ´{{ $cat->name }}ª?'" data-confirm-danger="true" data-loader>
                                                @csrf @method('DELETE')
