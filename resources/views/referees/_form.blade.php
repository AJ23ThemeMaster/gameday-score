<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <x-text-input id="first_name" name="first_name" type="text" class="block mt-1 w-full"
                      :value="old('first_name', $referee->first_name ?? '')" required autofocus />
    ...
    <div>
        ...
        <x-text-input id="last_name" name="last_name" type="text" class="block mt-1 w-full"
                      :value="old('last_name', $referee->last_name ?? '')" required />
    ...
    <div>
        ...
        <x-text-input id="document_id" name="document_id" type="text" class="block mt-1 w-full"
                      :value="old('document_id', $referee->document_id ?? '')" />
    ...
    <div>
        <x-input-label for="certification" :value="__('Certificación')" />
        <x-text-input id="certification" name="certification" type="text" maxlength="120"
                      class="block mt-1 w-full" :value="old('certification', $referee->certification ?? '')"
                      placeholder="Ej: Federación Venezolana de Béisbol - Nivel 2" />
    ...
    <div>
        ...
        <x-text-input id="experience_years" name="experience_years" type="number" min="0" max="99"
                      class="block mt-1 w-full" :value="old('experience_years', $referee->experience_years ?? '')" />
        <x-input-error :messages="$errors->get('experience_years')" class="mt-2" />
    </div>

    <div class="md:col-span-2">
        <x-input-label for="photo" :value="__('Foto del árbitro')" />
        @if (! empty($referee?->photo_path) && Storage::disk('public')->exists($referee->photo_path))
            <div class="mt-2 flex items-center gap-4">
                <img src="{{ $referee->photoUrl }}" class="h-20 w-20 object-cover bg-wv-surface rounded-full border border-wv-border p-1">
    ...
    <div class="md:col-span-2">
        ...
        <textarea id="notes" name="notes" rows="3"
                  class="block mt-1 w-full border-wv-border bg-wv-surface text-wv-text placeholder-wv-text-secondary focus:border-wv-accent focus:ring-wv-accent rounded-md shadow-sm">{{ old('notes', $referee->notes ?? '') }}</textarea>
        <x-input-error :messages="$errors->get('notes')" class="mt-2" />
    </div>

    <div class="md:col-span-2 flex items-center">
        <input id="active" name="active" type="checkbox" value="1"
               {{ old('active', $referee->active ?? true) ? 'checked' : '' }}
